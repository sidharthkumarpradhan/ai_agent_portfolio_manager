...
class DataFetcher:
    def __init__(self):
        # Force use the new API key
        self.alpha_vantage_key = '5FWTI1FGA0QOGQK4'
        self.finnhub_key = "d1e7p0pr01qlt46t5sv0d1e7p0pr01qlt46t5svg"
        ...
        self.alpha_vantage_exhausted = False
        self.api_call_count = 0
        self.logger.info(f"Using fresh Alpha Vantage API key: {self.alpha_vantage_key}")
        ...
        try:
            ...
            if self.db.data_freshness_check(symbol, 'stock', max_age_hours=1):
                ...
                if cached_data is not None and not cached_data.empty:
                    ...
                    df = cached_data.set_index('timestamp')[['open_price', 'high_price', 'low_price', 'close_price', 'volume']]
            ...
            if self.alpha_vantage_exhausted or not self.alpha_vantage_key or self.api_call_count >= self.max_alpha_vantage_calls:
                self.logger.info(f"Using Finnhub for {symbol} (Alpha Vantage exhausted or unavailable)")
                return self._get_stock_data_finnhub(symbol)
            else:
                self.logger.info(f"Trying Alpha Vantage for {symbol} (call #{self.api_call_count + 1})")
                result = self._get_stock_data_alpha_vantage(symbol)
                if result.empty:
                    self.logger.warning(f"Alpha Vantage failed for {symbol}, switching to Finnhub")
                    return self._get_stock_data_finnhub(symbol)
                return result
    ...
    def _get_stock_data_alpha_vantage(self, symbol):
        ...
        try:
            ...
            if 'Information' in data:
                self.logger.warning(f"Alpha Vantage rate limit hit: {data['Information']}")
            ...
            if 'Note' in data:
                self.logger.warning(f"Alpha Vantage note for {symbol}: {data['Note']}")
                self.alpha_vantage_exhausted = True
                return pd.DataFrame()
            
            time_series = data.get('Time Series (Daily)', {})
            if not time_series:
                self.logger.warning(f"No time series data from Alpha Vantage for {symbol}")
                return pd.DataFrame()
    ...
    def _get_stock_data_finnhub(self, symbol):
        ...
        try:
            ...
            self.logger.info(f"Successfully fetched {symbol} from Finnhub (real current price: ${current_price})")
            return df
    ...
    def _get_fallback_data(self, symbol):
        ...
        try:
            ...
            self.logger.error(f"Error getting fallback data for {symbol}: {str(e)}")
            return pd.DataFrame()
    ...
    def get_crypto_data(_self, symbol):
        ...
        try:
            ...
            if _self.db.data_freshness_check(symbol, 'crypto', max_age_hours=1):
                cached_data = _self.db.get_market_data(symbol, 'crypto', days_back=30)
                if cached_data is not None and not cached_data.empty:
                    return cached_data.set_index('timestamp')[['open_price', 'high_price', 'low_price', 'close_price', 'volume']].rename(columns={
            ...
            if 'Error Message' in data:
                st.error(f"Error fetching crypto data for {symbol}: {data['Error Message']}")
                return pd.DataFrame()
            
            if 'Note' in data:
                st.warning("API call frequency limit reached. Please try again later.")
                return pd.DataFrame()
    ...
    def get_crypto_data(self, symbol):
        ...
        try:
            ...
            self.logger.info(f"Skipping database cache, fetching fresh data for {symbol}")
            ...
            self.logger.error(f"Error fetching crypto data for {symbol}: {str(e)}", exc_info=True)
            return pd.DataFrame()
    ...
    def _get_coingecko_data(self, coin_id, original_symbol):
        ...
        try:
            ...
            if response.status_code != 200:
                self.logger.error(f"CoinGecko API error {response.status_code} for {coin_id}")
                return pd.DataFrame()
            
            data = response.json()
            
            if 'prices' not in data or not data['prices']:
                self.logger.warning(f"No price data from CoinGecko for {coin_id}")
                return pd.DataFrame()
            ...
            for i, price_point in enumerate(prices):
                ...
                high_price = price + abs(np.random.normal(0, daily_volatility * 0.5))
                low_price = price - abs(np.random.normal(0, daily_volatility * 0.5))
                close_price = price
            ...
            dates = pd.date_range(end=pd.Timestamp.now().normalize(), periods=len(df_data), freq='D')
            ...
            try:
                ...
                df_for_db['timestamp'] = df_for_db['timestamp'].dt.strftime('%Y-%m-%d %H:%M:%S')
                ...
                self.logger.info(f"Attempting to store {original_symbol} data in database")
                self.db.store_market_data(original_symbol, 'crypto', df_for_db)
                self.logger.info(f"Successfully stored {original_symbol} data in database")
            except Exception as db_error:
                self.logger.error(f"Database storage failed for {original_symbol}: {str(db_error)}", exc_info=True)
                # Continue anyway - don't fail the whole function for database issues
            
            price = df['close'].iloc[-1]
            self.logger.info(f"Successfully fetched {original_symbol} from CoinGecko: ${price:,.2f}")
            return df
    ...
    def _get_defillama_data(self, symbol):
        ...
        try:
            ...
            if not defillama_id or 'coins' not in data or defillama_id not in data['coins']:
            ...
            for i, date in enumerate(dates):
                ...
                high_price = close_price + abs(np.random.normal(0, daily_vol * 0.5))
                low_price = close_price - abs(np.random.normal(0, daily_vol * 0.5))
            ...
            try:
                ...
                self.logger.info(f"Attempting to store {symbol} data in database")
                self.db.store_market_data(symbol, 'crypto', df_for_db)
                self.logger.info(f"Successfully stored {symbol} data in database")
            except Exception as db_error:
                self.logger.error(f"Database storage failed for {symbol}: {str(db_error)}", exc_info=True)
                # Continue anyway - don't fail the whole function for database issues
            
            self.logger.info(f"Successfully fetched {symbol} from DefiLlama: ${current_price:,.2f}")
            return df
    ...
    def get_volatility_data(self, symbols, period_days=30):
        ...
        for symbol in symbols:
            try:
                ...
                st.warning(f"Could not calculate volatility for {symbol}: {str(e)}")
